...
public class TestClock : IPreciseClock
{
    ...
    /// <summary>
    /// Gets the actual current instant, based on the <paramref name="precision"/> specified, or the current <see cref="Precision"/>.
    /// </summary>
    ...
    /// <summary>
    /// Creates a <see cref="TestClock"/> that increases by <paramref name="interval"/>, starting with <paramref name="firstInstant"/>, each time <see cref="GetCurrentInstant"/> is called.
    /// </summary>
        ...
        ClockPrecision precision = ClockPrecision.Standard)
    {
        ...
        return new(i => i + d, precision, firstInstant.Value - d);
    }
